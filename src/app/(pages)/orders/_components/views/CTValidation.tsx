...
import { Descriptions, Divider, Image, Spin, Alert, Flex } from "antd";
...
const CTScan: React.FC<{ orderId: string }> = ({ orderId }) => {
	...
	const items2 = [
		...
		{ key: "5", label: "Date", children: data?.createdAt ? <DateDisplay isoDate={data.createdAt} /> : "N/A" },
	];
			...
			<Flex wrap justify="space-evenly">
				...
				>
					<Image
						...
						src={data?.ctImage2D}
						alt=""
						preview={data?.ctImage2D ? true : false}
					/>
					<Image
						width={200}
						src={data?.ctImage3D}
						alt=""
						preview={data?.ctImage3D ? true : false}
					/>
				</Image.PreviewGroup>
			</Flex>
		</>
